        ...
        for(columna=0; columna<Img1->width; columna+=tamBloque){
            ...
            for(filaCMP=0; filaCMP<Img2->height; filaCMP+=tamBloque){
                for(columnaCMP=0; columnaCMP<Img2->width; columnaCMP+=tamBloque){
                    int resComp = compararBloques(fila, columna, Img1, filaCMP, columnaCMP, Img2, Img1->height, Img1->width);
                    printf("%d\n", resComp);
                    if(resComp<minDiff){
                        minDiff = resComp;
            ...
            copiarBloque(fila, columna, Img1, minBloqueFil, minBloqueCol, Img2, Img2->height, Img2->width);
            cvWaitKey(0);
            cvShowImage(argv[2], Img2);
        }
    }
